...
class OW_Diff {
   ...
   * $compare_characters is true), and one of the constants DIFF::UNMODIFIED (the
   ...
   * $compare_characters - true to compare characters, and false to compare
   *                      lines; this optional parameter defaults to false
   */
  public function compare(
      $string1, $string2, $compare_characters = false){
    ...
    $table = $this->compute_table($sequence1, $sequence2, $start, $end1, $end2);

    // generate the partial diff
    $partialDiff = $this->generate_partial_diff($table, $sequence1, $sequence2, $start);
    ...
    for ($index = $end1 + 1;
        $index < ($compare_characters ? strlen($sequence1) : count($sequence1));
        $index ++){
      $diff[] = array($sequence1[$index], self::UNMODIFIED);
    }
  ...
  public function compare_files(
      $file1, $file2, $compare_characters = false){
   ...
   */
  private function compute_table(
   ...
   * $start     - the starting index
   */
  private function generate_partial_diff(
   ...
   *              to "\n"
   */
  public function to_string($diff, $separator = "\n"){
   ...
   *              to '<br>'
   */
  public function to_html($diff, $separator = '<br>'){
   ...
   *                defaults to '<br>'
   */
  public function to_table($diff, $indentation = '', $separator = '<br>'){
  ...
  /* Returns the content of the cell, for use in the to_table function. The
   * parameters are:
   *
   * $diff        - the diff array
   ...
   */
  private function get_cell_content(
